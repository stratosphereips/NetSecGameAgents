...
# This agents does normal actions following a set of probability distributions
    ...
    def play_game(self, num_episodes=1):
        ...
        for episode in range(num_episodes):
            ...
            while observation and not observation.end:
                ...
                # select the action 
                action, action_time = self.select_action(observation)
                ...
                if self._apm_limit:
                    ...
                    if remaining_time > 0:
                        ...
                        self._logger.debug(f"Waiting for {action_time}s before next action")
...
if __name__ == '__main__':
    ...
    logging.basicConfig(filename=os.path.join(args.logdir, "benign_agent.log"), filemode='w', format='%(asctime)s %(name)s %(levelname)s %(message)s', datefmt='%H:%M:%S',level=logging.DEBUG)

    # Create agent
    agent = BenignAgent(args.host, args.port,"Human",allowed_actions=[ActionType.FindData, ActionType.ExfiltrateData, ActionType.FindServices], apm_limit=args.apm)
