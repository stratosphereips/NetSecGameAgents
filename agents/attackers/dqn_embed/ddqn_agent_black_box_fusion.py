...
from agents.agent_utils import generate_valid_actions, state_as_ordered_string, filter_log_files_from_state, convert_ips_to_concepts
...
class FusedQNetwork(nn.Module):
    def __init__(self, state_dim, action_dim):
        ...
        self.fusion_net = nn.Sequential(
            nn.Linear(state_dim + state_dim, 1024), # Combine inputs
            nn.ReLU(),
            nn.Linear(1024, 512),    # Project down to the desired state dimension
            nn.ReLU()
        )

        self.net = nn.Sequential(
            nn.Linear(512 + action_dim, 1024),
            nn.ReLU(),
            nn.Linear(1024, 256),
            # nn.ReLU(),
            # nn.Linear(512, 256),
    ...
    def forward(self, state_emb, concept_emb, action_emb):
        if state_emb.dim() == 1:
            state_emb = state_emb.unsqueeze(0)
            action_emb = action_emb.unsqueeze(0)
            
        # 1. State Concatenation
        E_state_concat = torch.cat((state_emb, concept_emb), dim=1)
        
        # 2. State Fusion (Projection)
        S_fused = self.fusion_net(E_state_concat)
        
        # 3. State-Action Concatenation for DQN input
        S_A_concat = torch.cat((S_fused, action_emb), dim=1)
        
        # 4. Q-Value Prediction
        return self.net(S_A_concat).squeeze(-1)
...
class DDQNAgent(BaseAgent):
    ...
    def define_networks(self):
        self.q_net = FusedQNetwork(self.state_encoder.embedding_dim, self.action_encoder.embedding_dim)
        self.target_net = FusedQNetwork(self.state_encoder.embedding_dim, self.action_encoder.embedding_dim)
    ...
    def generate_concept_embedding(self, observation: Observation) -> torch.Tensor:
        # obs = filter_log_files_from_state(observation)
        concept_obs = convert_ips_to_concepts(observation, self._logger)
        state_str = state_as_ordered_string(concept_obs.observation.state)
        
        if state_str not in self.concept_set.keys():
            # save the
            embedding = self.state_encoder.encode(state_str).detach().float().cpu()
            self.concept_set[state_str] = embedding
            return embedding
        else:
            return self.concept_set[state_str]

    @torch.no_grad()
    def select_action(self, observation: Observation, epsilon: float):

        concept_emb = self.generate_concept_embedding(observation).to(self.device)
        ...
        concept_emb_expanded = concept_emb.repeat(num_valid_actions, 1)
    
        # 2. Compute Q-values for all (s, a_valid) pairs
        # The net will take (N_valid, D_S) and (N_valid, D_A)
        q_values = self.q_net(state_emb_expanded, concept_emb_expanded, valid_action_embs) # (N_valid,)
    ...
    def update(self, episode:int, batch_size:int=32, num_epochs:int=1):
        ...
        for _ in range(num_epochs):
            ...
            for observation, action_emb_taken, reward, next_observation, done in batch:
                ...
                concept_emb = self.generate_concept_embedding(observation)
                ...
                next_concept_emb = self.generate_concept_embedding(next_observation)
                next_concept_embs.append(next_concept_emb)
                
                # ---- Double DQN target Q ----
                with torch.no_grad():
                    # --- Double DQN Target Q ---
                    # This requires iterating through the batch since the number of valid actions varies.
                    # 1. Online net selects the best next action (action a') for the next state s'
            ...
            concept_batch = torch.stack(concept_embs).to(self.device)
            # next_state_batch = torch.stack(next_state_embs).to(self.device)
            # next_concept_batch = torch.stack(next_concept_embs).to(self.device)
            ...
            q_values = self.q_net(state_batch, concept_batch, actions_emb_taken_batch) # (B,)
            ...
            #         # 1. Online net selects the best next action (action a') for the next state s'
            #         _, best_action_emb_online = self.select_action(b[3], 0.0) # (D_A)

            #         # 2. Target net evaluates Q(s', a')
            #         # Use the target network to evaluate the action selected by the online network
            #         next_q_value = self.target_net(
            #             next_state_batch[i].unsqueeze(0),
            #             next_concept_batch[i].unsqueeze(0),
            #             best_action_emb_online.unsqueeze(0) # Need (1, D_A)
            #         ).squeeze(0) # scalar
...
if __name__ == "__main__":
    ...
    wandb.init(
        ...
        entity="stratosphere",
        name="DDQN-embed-black-box-fusion",
        config={
