...
class ProbabilisticDefenderAgent(BaseAgent):
    def __init__(self, host:str, port:int, role:str, allowed_actions:list, apm_limit:int=None) -> None:
        ...
        if self._apm_limit:
            self.interaction_interval = 60/apm_limit
        else:
        ...
        for episode in range(num_episodes):
            ...
            while observation and not observation.end:
                ...
                if self._apm_limit:
                    ...
                    remaining_time = self.interaction_interval - elapsed_time
                    if remaining_time > 0:
...
if __name__ == '__main__':
    ...
    logging.basicConfig(filename=os.path.join(args.logdir, "probabilistic_defender_agent.log"), filemode='w', format='%(asctime)s %(name)s %(levelname)s %(message)s', datefmt='%H:%M:%S',level=logging.DEBUG)

    # Create agent
    agent = ProbabilisticDefenderAgent(args.host, args.port, "Defender", allowed_actions=[ActionType.FindData, ActionType.BlockIP], apm_limit=args.apm)
